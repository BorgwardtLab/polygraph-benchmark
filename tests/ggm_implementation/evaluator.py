from ggm_implementation import gin_evaluation, graph_structure_evaluation
...
class Evaluator:
    def __init__(self, feature_extractor="gin", **kwargs):
        if feature_extractor != "mmd-structure":
            ...
            self.evaluators = []
            self.evaluators.append(gin_evaluation.FIDEvaluation(model=model))
            # self.evaluators.append(gin_evaluation.KIDEvaluation(
            #    model=model))
            self.evaluators.append(
                gin_evaluation.prdcEvaluation(model=model, use_pr=True)
            )
            self.evaluators.append(
                gin_evaluation.prdcEvaluation(model=model, use_pr=False)
            )
            self.evaluators.append(
                gin_evaluation.MMDEvaluation(
                    model=model, kernel="rbf", sigma="range", multiplier="mean"
                )
            )
            self.evaluators.append(
                gin_evaluation.MMDEvaluation(model=model, kernel="linear")
            )
            ...
            try:
                ...
                self.logger.warning(f"Failed to get number of parameters: {e}")
                pass

        elif (
            feature_extractor == "mmd-structure"
            and kwargs.get("statistic") != "WL"
            and kwargs.get("statistic") != "nspdk"
        ):
            ...
            feature_extractor == "mmd-structure" and kwargs.get("statistic") == "nspdk"
        ):
            self.evaluators = [graph_structure_evaluation.NSPDKEvaluation()]
        else:
            raise Exception(
                "Unsupported feature extractor {} or statistic {}".format(
                    kwargs.get("feature_extractor"), kwargs.get("statistic")
                )
            )

    def evaluate_all(self, generated_dataset=None, reference_dataset=None, **kwargs):
        metrics = {}
        if len(self.evaluators) > 2:
            (generated_dataset, reference_dataset), time = self.evaluators[
                0
            ].get_activations(generated_dataset, reference_dataset)
