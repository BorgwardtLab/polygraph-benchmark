...
from graph_gen_gym.datasets import ProceduralLobsterGraphDataset, ProceduralPlanarGraphDataset, ProceduralSBMGraphDataset
...
def test_split_disjointness(ds_cls):
    ...
    assert metrics["novel"].mle == 1.0
    assert metrics["unique"].mle == 1.0
    if ds_cls != ProceduralSBMGraphDataset:
        assert metrics["valid"].mle == 1.0
    else:
        assert metrics["valid"].mle >= 0.7
