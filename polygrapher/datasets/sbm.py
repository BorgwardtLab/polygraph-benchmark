...
def is_sbm_graph(graph: nx.Graph, intra_p: float = 0.3, inter_p: float = 0.005, min_n_communities: int = 2, max_n_communities: int = 5, min_n_nodes_per_community: int = 20, max_n_nodes_per_community: int = 40) -> bool:
    ...
    W_p_intra = (est_p_intra - intra_p) ** 2 / (est_p_intra * (1 - est_p_intra) + 1e-6)
...
def is_sbm_graph_alt(graph: nx.Graph, intra_p: float = 0.3, inter_p: float = 0.005, min_n_communities: int = 2, max_n_communities: int = 5, min_n_nodes_per_community: int = 20, max_n_nodes_per_community: int = 40) -> bool:
    partition_methods = [
        lambda g: {
            node: cluster
    ...
    if any(count < min_n_nodes_per_community or count > max_n_nodes_per_community for count in node_counts.values()):
        return False
...
class ProceduralSBMGraphDataset(ProceduralGraphDataset):
    """Procedural version of [`SBMGraphDataset`][polygrapher.datasets.SBMGraphDataset].
    ...
    """
    def __init__(self, split: Literal["train", "val", "test"], num_graphs: int, seed: int = 42, intra_p: float = 0.3, inter_p: float = 0.005, n_communities: Tuple[int, int] = (2, 5), n_nodes_per_community: Tuple[int, int] = (20, 40), memmap: bool = False):
        config_hash = joblib.hash((num_graphs, intra_p, inter_p, n_communities, n_nodes_per_community, seed, split), hash_name='md5')
    ...
    def is_valid(self, graph: nx.Graph) -> bool:
        ...
        return is_sbm_graph(graph, intra_p=self._intra_p, inter_p=self._inter_p, min_n_communities=self._n_communities[0], max_n_communities=self._n_communities[1], min_n_nodes_per_community=self._n_nodes_per_community[0], max_n_nodes_per_community=self._n_nodes_per_community[1])

    def is_valid_alt(self, graph: nx.Graph) -> bool:
        return is_sbm_graph_alt(graph, intra_p=self._intra_p, inter_p=self._inter_p, min_n_communities=self._n_communities[0], max_n_communities=self._n_communities[1], min_n_nodes_per_community=self._n_nodes_per_community[0], max_n_nodes_per_community=self._n_nodes_per_community[1])
    ...
    def _random_sbm(self):
        ...
        num_nodes_per_community = self._rng.integers(
            self._n_nodes_per_community[0], self._n_nodes_per_community[1] + 1, size=num_communities
        )
